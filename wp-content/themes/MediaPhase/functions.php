...
if ( !function_exists( 'mediaphase_setup' ) ) :
	...
	function mediaphase_setup()
	{
		 ...
		 * to change 'mediaphase' to the name of your theme in all the template files
		 */
		load_theme_textdomain( 'mediaphase', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'primary' => __( 'Primary Menu', 'mediaphase' ),
			'social'  => __( 'Social', 'mediaphase' ),
		) );

		// Add default items to primary menu
		$default_primary_menu_created = get_option( 'default_primary_menu_created' );
		if ( !$default_primary_menu_created ) {
			$name = 'primary';
			$menu_id = wp_create_nav_menu( $name );
			$menu = get_term_by( 'name', $name, 'nav_menu' );

			wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'Home', 'mediaphase' ),
					'menu-item-url'    => home_url( '/' ),
					'menu-item-status' => 'publish' )
			);

			$product_id = wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'Products', 'mediaphase' ),
					'menu-item-url'    => home_url( '/' ),
					'menu-item-status' => 'publish' )
			);

			wp_update_nav_menu_item( $menu->term_id, 0, array(
				'menu-item-title'     => __( 'Product 1', 'mediaphase' ),
				'menu-item-url'       => home_url( '/' ),
				'menu-item-status'    => 'publish',
			...
			wp_update_nav_menu_item( $menu->term_id, 0, array(
				'menu-item-title'     => __( 'Product 2', 'mediaphase' ),
				'menu-item-url'       => home_url( '/' ),
				'menu-item-status'    => 'publish',
				'menu-item-parent-id' => $product_id,
			) );
			...
			wp_update_nav_menu_item( $menu->term_id, 0, array(
				'menu-item-title'     => __( 'Product 3', 'mediaphase' ),
				'menu-item-url'       => home_url( '/' ),
				'menu-item-status'    => 'publish',
				'menu-item-parent-id' => $product_id,
			) );
			...
			wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'About', 'mediaphase' ),
					'menu-item-url'    => home_url( '/' ),
					'menu-item-status' => 'publish' )
			);

			wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'Contact', 'mediaphase' ),
					'menu-item-url'    => home_url( '/' ),
					'menu-item-status' => 'publish' )
			);
			...
			update_option( 'default_primary_menu_created', true );
		}

		// Add default items to social menu
		$default_social_menu_created = get_option( 'default_social_menu_created' );
		if ( !$default_social_menu_created ) {
			$name = 'social';
			$menu_id = wp_create_nav_menu( $name );
			$menu = get_term_by( 'name', $name, 'nav_menu' );

			wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'Twitter', 'mediaphase' ),
					'menu-item-url'    => 'http://twitter.com/themefurnace',
					'menu-item-status' => 'publish' )
			);

			wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'Wordpress', 'mediaphase' ),
					'menu-item-url'    => 'http://wordpress.com',
					'menu-item-status' => 'publish' )
			);

			wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'Facebook', 'mediaphase' ),
					'menu-item-url'    => 'http://facebook.com/themefurnace',
					'menu-item-status' => 'publish' )
			);

			wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'Dribbble', 'mediaphase' ),
					'menu-item-url'    => 'http://dribbble.com/themefurnace',
					'menu-item-status' => 'publish' )
			);

			wp_update_nav_menu_item( $menu->term_id, 0, array(
					'menu-item-title'  => __( 'Google+', 'mediaphase' ),
					'menu-item-url'    => 'http://plus.google.com/+Wplift',
					'menu-item-status' => 'publish' )
			);
			...
			set_theme_mod( 'nav_menu_locations', $locations );
			update_option( 'default_social_menu_created', true );
		}
...
function mediaphase_widgets_init()
{
	register_sidebar( array(
		'name'          => __( 'Sidebar', 'mediaphase' ),
		'id'            => 'mediaphase-sidebar',
		'description'   => '',
	...
	register_sidebar( array(
		'name'          => __( 'Main Features Left', 'mediaphase' ),
	...
	register_sidebar( array(
		'name'          => __( 'Main Features Right', 'mediaphase' ),
	...
	register_sidebar( array(
		'name'          => __( 'Sub Features', 'mediaphase' ),
...
{
	$fonts_url = '';
	$roboto = _x( 'on', 'Roboto font: on or off', 'mediaphase' );
...
{
	if ( !get_option( 'mediaphase_content_set' ) ) {

		// set up default widgets
		$active_sidebars = get_option( 'sidebars_widgets' );
		$search_widget = get_option( 'widget_search' );
		$search_widget[1] = array( 'title' => __( 'Search', 'mediaphase' ) );

		$categories_widget = get_option( 'widget_categories' );
		$categories_widget[1] = array( 'title' => __( 'Categories', 'mediaphase' ), 'count' => 1 );

		$recent_posts_widget = get_option( 'widget_recent-posts' );
		$recent_posts_widget[1] = array( 'title' => __( 'Recent Posts', 'mediaphase' ), 'show_date' => 1, 'number' => 5 );
		$recent_posts_widget[2] = array( 'title' => __( 'Recent Posts', 'mediaphase' ), 'show_date' => 1, 'number' => 5 );

		$recent_comments_widget = get_option( 'widget_recent-comments' );
		$recent_comments_widget[1] = array( 'title' => __( 'Recent Comments', 'mediaphase' ) );

		$meta_widget = get_option( 'widget_meta' );
		$meta_widget[1] = array( 'title' => __( 'Meta', 'mediaphase' ) );
		...
		$active_sidebars['mediaphase-sidebar'] = array( 'recent-posts-2', 'text-1', 'search-1', 'nav_menu-1', 'categories-1' );
		...
		set_theme_mod( 'mediaphase_header_logo_text', 'Mediaphase' );
		set_theme_mod( 'mediaphase_header_contacts_phone', '0800 123 4567' );
		set_theme_mod( 'mediaphase_header_contacts_email', 'contact@themefurnace.com' );
...
<a href="#" class="herobutton clear">Join Now <i class="fa fa-chevron-circle-right"></i></a>' );
		...
		update_option( 'mediaphase_content_set', 1 );
	}
}

add_action( 'wp_loaded', 'mediaphase_set_sample_content', 100 );
...
/**
 * Upsell links and content
 */
require get_template_directory() . '/inc/mediaphase-upsell.php';
