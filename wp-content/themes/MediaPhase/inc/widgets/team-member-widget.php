...
class Mediaphase_Team_Member_Widget extends WP_Widget
{
	...
	public function widget( $args, $instance )
	{
		...
		if ( !empty( $social[0] ) || !empty( $social[1] ) || !empty( $social[2] ) || !empty( $social[3] ) || !empty( $social[4] ) ) {
			...
			foreach ( $social as $url ) {
				if ( !empty( $url ) ) {
					echo '<a href="' . $url . '"><span class="screen-reader-text">' . $url . '</span></a>';
				}
			}
			echo '</div>';
		}
	...
	public function form( $instance )
	{
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'mediaphase' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'textbox' ); ?>"><?php _e( 'Text:', 'mediaphase' ); ?></label>
		...
		<p>
			<label
				for="<?php echo $this->get_field_id( 'social_twitter' ); ?>"><?php _e( 'Twitter url', 'mediaphase' ); ?></label>
			...
			<label
				for="<?php echo $this->get_field_id( 'social_facebook' ); ?>"><?php _e( 'Facebook url', 'mediaphase' ); ?></label>
			...
			<label
				for="<?php echo $this->get_field_id( 'social_dribbble' ); ?>"><?php _e( 'Dribbble url', 'mediaphase' ); ?></label>
			...
			<label
				for="<?php echo $this->get_field_id( 'social_linkedin' ); ?>"><?php _e( 'Linkedin url', 'mediaphase' ); ?></label>
			...
			<label
				for="<?php echo $this->get_field_id( 'social_gplus' ); ?>"><?php _e( 'Google+ url', 'mediaphase' ); ?></label>
		...
		<p>
			<label
				for="<?php echo $this->get_field_id( 'image_url' ); ?>"><?php _e( 'Image:', 'mediaphase' ); ?></label><br/>
			<input type="text" class="img" name="<?php echo $this->get_field_name( 'image_url' ); ?>"
				   id="<?php echo $this->get_field_id( 'image_url' ); ?>"
				   value="<?php echo esc_attr( $image_url ); ?>"/>
			<input type="button" class="mediaphase-select-img" value="<?php _e( 'Select Image', 'mediaphase' ); ?>"/>
		</p>
	<?php
	}
